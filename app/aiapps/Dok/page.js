...
export default function Index() {
  ...
  const items = [
    { label: `${AppTitle.dok}`, url: "" },
  ];
  const home = { label: "BrixAI Apps", url: "/aiapps" };
  const [selectedGrade, setSelectedGrade] = useState(null);
  const [dokLevel, setDokLevel] = useState(null);
  ...
  const dokLevelRef = useRef(null);

  const handleExample = () => {
    setSelectedGrade(GRADE[8])
    setDokLevel(DokLevel[0])
    setStandard(STANDARD[1])
    setTopic("How do food webs describe the ways in which animals in an ecosystem are related?")
    setStandardObjective("How do food webs describe the ways in which animals in an ecosystem are related?")
    setFormShow(true);
  ...
  const validate = () => {
    ...
    else if (!dokLevel || dokLevel.name.trim() === "") {
      // err.topic = 'Please Enter Topic, Standard or Objective.';
      toast.error('Please Select Dok Level.')
      isErr = true;
    }
    ...
    else if (!standardObjective || standardObjective.trim() === "") {
      // err.standard = 'Please Select Standards Set to Align to.';
      toast.error('Please Enter Standard or Objective.')
      isErr = true;
    }
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      ...
      dokLevel: dokLevel ? dokLevel.name : null,
    };
              ...
              {loading ? <div className="flex justify-center items-center" >
                ...
                <form>
                  <div className="grid grid-cols-6 xl:gap-[1.25vw] gap-[18px] mx-0 mb-3">
                    ...
                    <div className="grid col-span-2">
                      <label className="3xl:text-[0.729vw] text-[14px] text-[#344054] font-medium block mb-[6px]">
                      DOK level:<span className="text-[red] ml-1">*</span>
                      </label>
                      <Dropdown
                        value={dokLevel}
                        onChange={(e) => setDokLevel(e.value)}
                        options={DokLevel}
                        optionLabel="name"
                        placeholder="Select"
                        filter
                  ...
                  <div className="mb-2">
                    <label className="3xl:text-[0.729vw] text-[14px] text-[#344054] font-medium block mb-[6px]">
                      Topic:  <span className="text-[red] ml-1">*</span>
                    </label>
                    <InputTextarea
                      autoResize
                      placeholder="Type..."
                  ...
                  <div className="mb-2">
                    <label className="3xl:text-[0.729vw] text-[14px] text-[#344054] font-medium block mb-[6px]">
                      Standard or Objective:  <span className="text-[red] ml-1">*</span>
                    </label>
                    <InputTextarea
                      autoResize
                      placeholder="Type..."
                      value={standardObjective}
                      onChange={(e) => setStandardObjective(e.target.value)}
                      rows={3}
                      className="w-full"
                    />
                    {/*  {error.topic && <div className="text-red-500">{error.topic}</div>} */}
