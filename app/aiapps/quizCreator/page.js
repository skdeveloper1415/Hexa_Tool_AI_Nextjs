...
export default function Index() {
  const items = [{ label: `${AppTitle.quizCreator}`, url: "" }];
  const home = { label: "BrixAI Apps", url: "/aiapps" };

  const appId = AppId.quizCreator;
  ...
  const [title, setTitle] = useState(`${AppTitle.quizCreator}`);
  ...
  const handleAction = () => {
    setVisible(true);
    setExempler(false);
    setTitle(`Generated ${AppTitle.quizCreator}`);
  };
  ...
  return (
    <Layout pageTitle={AppTitle.quizCreator}>
