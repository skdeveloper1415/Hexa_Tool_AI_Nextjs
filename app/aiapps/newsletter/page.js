...
import { GRADE, STANDARD, AppId, AppTitle, AppDesc, DokLevel, AppCategory } from "../../../components/helper/enum";
...
export default function Index() {
  ...
  const handleExample = () => {
    ...
    setSelectedClassroom(GRADE[7])
    setDepartment(DokLevel[0])
    setTeam(STANDARD[0])
    setSchool(DokLevel[0])
    setFormShow(true);
  ...
  const validate = () => {
    ...
    if (!selectedClassroom || selectedClassroom.name.trim() === "") {
      // err.topic = 'Please Enter Topic, Standard or Objective.';
      toast.error('Please Select Classroom.')
      isErr = true;
    } else if (!selectedGrade || selectedGrade.name.trim() === "") {
      // err.grade = 'Please Select Grade Level.';
      toast.error('Please Select Grade Level.')
      isErr = true;
    }
    else if (!school || school.name.trim() === "") {
      toast.error('Please Select School.')
      isErr = true;
    } else if (!department || department.name.trim() === "") {
      toast.error('Please Select Department.')
      isErr = true;
    } else if (!team || team.name.trim() === "") {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    const payload = {
      grade: selectedGrade ? selectedGrade.name : null,
      teams: team.code,
      classroom: selectedClassroom ? selectedClassroom.name : null,
    ...
    <Layout>
          ...
          <div className="xl:col-span-10 lg:col-span-10 col-span-12 bg-[#fff] border border-[#E4E7EC] xl:p-[1.25vw] p-[10px] xl:mx-[3.51vw] lg:mx-[40px] mx-0 rounded-lg shadow-lg">
            {formShow && (<>
...

                {
                  isShowHide == true ? (isShowHide && !loading &&
                    ...
                    >
                      ...
                      Hide  Prompt
                    </button>) : <div className="3xl:text-[0.938vw] 2xl:text-[16px] text-[12px] font-medium mb-10">Category: {AppCategory.qualityCommunication}</div>
              ...
              {loading ? <div className="flex justify-center items-center" >
                ...
                <form>
                  <div className="grid grid-cols-6 xl:gap-[1.25vw] gap-[18px] mx-0 mb-5">
                    <div className="grid col-span-3">
                      <label className="3xl:text-[0.729vw] text-[14px] text-[#344054] font-medium block mb-[6px]">
                        Classroom:<span className="text-[red] ml-1">*</span>
                      </label>
                      <Dropdown
                    ...
                    <div className="grid col-span-2">
                      <label className="3xl:text-[0.729vw] text-[14px] text-[#344054] font-medium block mb-[6px]">
                        School:<span className="text-[red] ml-1">*</span>
                      </label>
                      <Dropdown
                        value={school}
                    ...
                    <div className="grid col-span-2">
                      <label className="3xl:text-[0.729vw] text-[14px] text-[#344054] font-medium block mb-[6px]">
                        Department:<span className="text-[red] ml-1">*</span>
                      </label>
                      <Dropdown
                        value={department}
                    ...
                    <div className="grid col-span-2">
                      <label className="3xl:text-[0.729vw] text-[14px] text-[#344054] font-medium block mb-[4px]">
                        Team:<span className="text-[red] ml-1">*</span>
                      </label>
                      <Dropdown
                        value={team}
      ...
      {visible &&
        <CommonActionExempler
          title={`Generated ${AppTitle.newsletter}`}
          response={createResponse}
          visible={visible}
          position={position}
